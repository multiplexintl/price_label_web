...
class AccessoryLabelNoPromoController extends GetxController {
  ...
  var data = List<AccessoryNoPromo>.generate(
      0,
      (index) => AccessoryNoPromo(
            brandName: "eylure",
  ...
  void saveOrUpdatePriceLabelPromo() {
    ...
    // Update or Create
    if (isEditMode.value) {
      // Update existing AccessoryNoPromo
      final index = currentEditIndex.value;
      if (index >= 0 && index < data.length) {
        data[index] = AccessoryNoPromo(
            brandName: brandName, barcode: barcode, price: price);
        log("Updated AccessoryNoPromo at index $index: ${data[index]}");
      } else {
        log("Error: Invalid index for update.");
      }
    } else {
      // Create new AccessoryNoPromo
      final newPromo = AccessoryNoPromo(
        brandName: brandName,
        barcode: barcode,
        price: price,
      );
      data.add(newPromo);
      log("Added new AccessoryNoPromo: $newPromo");
    }
  ...
  void editPriceLabelPromo(int index) {
    ...
    // Get the AccessoryNoPromo to edit
    ...
    log("Editing AccessoryNoPromo at index $index: $promo");
  ...
  void copyPriceLabelPromo(int index) {
    if (index >= 0 && index < data.length) {
      ...
      final copiedPromo = AccessoryNoPromo(
      ...
      log("Copied AccessoryNoPromo from index $index to index ${index + 1}: $copiedPromo");
